...
class PendingTaskForm:
    def __init__(self, master):
        self.master = master
        self.master.title("Pending Tasks")
        ...
        self.populate_Pending_tasks()

    def populate_Pending_tasks(self):
        Pending_tasks = self.fetch_Pending_tasks_from_database()

        if Pending_tasks:
            for task in Pending_tasks:
                self.tree.insert('', tk.END, values=task)
        else:
            messagebox.showinfo("No Pending Tasks", "No Pending tasks found.")

    def fetch_Pending_tasks_from_database(self):
        ...
        mycursor.execute("SELECT * FROM lists WHERE completed='no'")
        Pending_tasks = mycursor.fetchall()
